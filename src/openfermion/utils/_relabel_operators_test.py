...
class RelabelFermionOperatorTest(unittest.TestCase):
    """Class to test relabel functions."""
    ...
    def test_fermioperator(self):
        ...
        self.assertTrue(relabel_fermionoperator(op, 1) ==
                        FermionOperator())  # Return empty if frozen
        self.assertTrue(_relabel_single_fermi(op, 1, 4) == FermionOperator())
        self.assertTrue(relabel_fermionoperator(op, 0) == op)  # Do nothing
        self.assertTrue(relabel_fermionoperator(op3, 3) ==
                        op3)  # Empty operator doesn't change
        self.assertTrue(relabel_fermionoperator(
            op + op1 + op2 + op3, 1) == op1 + op3)  # Combined


class RelabelQubitOperatorTest(unittest.TestCase):
    """Class to test relabel function."""
    ...
    def test_qubitoperator(self):
        ...
        self.assertTrue(_relabel_single_pauli(op, 1, 4) ==
                        QubitOperator())  # Return empty if frozen
        self.assertTrue(relabel_qubitoperator(op, 0) == op)  # Do nothing
        # Empty operator doesn't change
        self.assertTrue(relabel_qubitoperator(op3, 3) == op3)
        self.assertTrue(relabel_qubitoperator(
            op + op1 + op2 + op3, 1) == op1 + op3)  # Combined
